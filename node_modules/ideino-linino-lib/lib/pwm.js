...
function Pwm(pin_register, timer, logger) {
	...
	if((!this.timer.ENA)){
		if(pin_register.MAX > this.period){
			fs.writeFileSync( periodPath, this.period );
		}
		else{
...
Pwm.prototype.writeSync = function(value) {
	if(this.timer.SHA && this.timer.ENA){
		value = getValues(value, fs.readFileSync(this.pwmPath + 'period'));
	}
	else{
