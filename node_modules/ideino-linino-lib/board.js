...
var	CHECK_MSG = {	PIN_UNDEFINED : 	"The specified pin is undefined!",
					...
					PIN_TIMER_SERVO:	"The timer used by specified pin [{0}] is used by a servo",
...
Board.prototype.tone = function() {
    try {
		var args = Args([
						{ pin: 			Args.STRING 	| Args.Required },
						{ period:   	Args.FLOAT 		| Args.Required },
					], arguments);
		
		var check = this.analogWritens(args.pin, args.period/2, args.period);
		if(check instanceof Error){
			throw check;
		}			
		return true;
    ...
    try {
		var args = Args([
			...
			if(check instanceof Error){
				...
				if( typeof(args.callback) == 'function' ){
					pin_register[args.pin].PIN.writens( args.value, args.period, function(val){
						args.callback(val);						
					});
				}
				else{
					pin_register[args.pin].PIN.writeSyncns(args.value,args.period);
					return args.value;
				}
			}
	}
