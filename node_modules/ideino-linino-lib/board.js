...
/////////////////////COMMENTO PER TEST 1
	...
	async.each(pin_pwm, function(value,cbk){
			if( fs.existsSync('/sys/class/mcuio_pwm/'+value.MAP) ){
		...
		checkPinMode(pin, mode, function(err){
			if(err){
				...
				switch(mode.toLowerCase()) {
					case 'input':
						pin_register[pin].PIN = new Gpio(pin_register[pin].NUM, 
													pin_register[pin].MAP,
													'in',
													'both');
						pin_register[pin].MOD = mode;
						break;
					case 'output':
						pin_register[pin].PIN = new Gpio(pin_register[pin].NUM, 
													pin_register[pin].MAP,
						...
						break;
					case 'pwm':
						pin_register[pin].PIN = new Pwm(pin_register[pin].NUM, 
													pin_register[pin].MAP);
						pin_register[pin].MOD = mode;
						break;
					case 'servo':
    ...
    try {
		var args = Args([
			...
			if(typeof(check) == 'error'){
				...
				if(args.value>=255) args.value = 255;
				if(args.value<=0) args.value = 0;
    ...
    try {		
		...
		if(typeof(check) == 'error'){
			...
			if(typeof(pin_register[args.pin].PIN) == 'undefined' || typeof(pin_register[args.pin].PIN) == {} ){
				...
				pin_register[args.pin].PIN = new Adc(pin_register[args.pin].NUM, 
												pin_register[args.pin].MAP,
												opts);
			}
			if(typeof(args.callback) != 'undefined'){
				pin_register[args.pin].PIN.watch(function(err, val){
...
function checkPinMode(pinnumber, mode, callback){
	try{	
		...
		if( !utils.contains(pin_layout.digital, pinnumber) && !utils.contains(pin_layout.pwm, pinnumber)){
...
function checkServoWrite(pinnumber,callback){	
	try{
		...
		if(pin_register[pinnumber].MOD.toLowerCase() != utils.MODES.SERVO.toLowerCase() ){
			callback(new Error( String.format(CHECK_MSG.MODE_NOT_VALID, pin_register[pinnumber].MOD.toLowerCase(), pinnumber) + " Allowed modes are: "+ utils.MODES.SERVO));return false;
		}
