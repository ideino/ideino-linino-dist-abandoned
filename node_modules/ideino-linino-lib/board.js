...
var	BUFFER_SIZE = 1000,			//items -> is the size of the buffer containing the write commands
	...
	//WAIT_BRIDGE_TIME = 15000,	//millis -> is the time before start the bridge
	BRIDGE_CONN_RETRIES = 15,	//int -> is the maximum number of bridge connection attemps
	ENCODING = 'utf8',
	SERVER = '127.0.0.1',
	PORT = 9810,	
...
Board.prototype.connect = function(callback){
	...
	async.series([
		function(cb){logger.debug("starting export analog adcs...");//export analog pins				
			gpio_a_export(function(err, status){
				if(err){
					...
					logger.debug("analog gpios export " + status);
					cb(null,"analog gpios export " + status);
				}
			});
		},
		function(cb){logger.debug("starting export analog gpios...");//export digital pins
			setTimeout(function(){
		...
		function(cb){logger.debug("start buffering...");
			setTimeout(function(){
			setInterval(function(){
				sendWriteRequest();
			},BUFFER_SPEED);
			cb(null,"buffering ok.");
			},500);
		},
	...
	/*
	//ciclo i pin digitali definiti nel layout
	callback(u.each( u.filter(layout2, function(pin){return pin.TYPE == 'digital';}),function(value, index, list){
		//logger.debug(value);
		gpio_d_export(value, function(err, status){
			if(err){
				logger.error("digital gpio " + value.GPIO + " export error" + err.message);
			}else{
				logger.debug("digital gpio " + value.GPIO + " export " + status);
			}
		});
	})
	);
